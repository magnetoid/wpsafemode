                ...
                <table class="md3-table">
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td><?php echo htmlspecialchars($data['health']['server']['server_software'] ?? 'Unknown'); ?>
                            </td>
                        </tr>
                        <tr>
            ...
            <?php if (isset($data['health']['database']) && $data['health']['database']['status'] === 'healthy'): ?>
                <div class="md3-stats-card">
                    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 16px;">
                        <div>
                            <div class="md3-stats-card-value"><?php echo $data['health']['database']['table_count'] ?? 0; ?>
                            </div>
                            <div class="md3-stats-card-label">Tables</div>
                        </div>
                        <div>
                            <div class="md3-stats-card-value">
                                <?php echo $data['health']['database']['total_size_formatted'] ?? '0 B'; ?></div>
                            <div class="md3-stats-card-label">Total Size</div>
                        </div>
                        <div>
                            <div class="md3-stats-card-value">
                                <?php echo count($data['health']['database']['fragmented_tables'] ?? []); ?></div>
                            <div class="md3-stats-card-label">Fragmented Tables</div>
                        </div>
                    </div>
                </div>

                <!-- Largest Tables -->
                <?php if (!empty($data['health']['database']['largest_tables'])): ?>
                    <div style="margin-top: 24px;">
                        <h4 style="margin-bottom: 12px; font-size: 1rem; color: var(--color-text-muted);">Top 10 Largest Tables</h4>
                        <div class="md3-table-container">
                            <table class="md3-table">
                                <thead>
                                    <tr>
                                ...
                                <tbody>
                                    <?php foreach ($data['health']['database']['largest_tables'] as $table): ?>
                                        <tr>
                                            <td><?php echo htmlspecialchars($table['name']); ?></td>
                                            <td><?php echo number_format($table['rows']); ?></td>
                                            <td><?php echo $table['formatted_size']; ?></td>
                                        </tr>
                                    <?php endforeach; ?>
                                </tbody>
                            </table>
            ...
            <?php if (isset($data['health']['wordpress'])): ?>
                <div class="md3-stats-card">
                    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 16px;">
                        <div>
                            <div class="md3-stats-card-value">
                                <?php echo $data['health']['wordpress']['wp_version'] ?? 'Unknown'; ?></div>
                            <div class="md3-stats-card-label">WordPress Version</div>
                        </div>
                        <div>
                            <div class="md3-stats-card-value"><?php echo $data['health']['wordpress']['active_plugins'] ?? 0; ?>
                            </div>
                            <div class="md3-stats-card-label">Active Plugins</div>
                        </div>
                        <div>
                            <div class="md3-stats-card-value"><?php echo $data['health']['wordpress']['total_themes'] ?? 0; ?>
                            </div>
            ...
            <?php if (isset($data['health']['disk']) && $data['health']['disk']['status'] !== 'unknown'): ?>
                <div>
                    ...
                    <div class="md3-linear-progress">
                        <div class="md3-linear-progress-bar"
                            style="width: <?php echo $data['health']['disk']['percent_used'] ?? 0; ?>%;"></div>
                    </div>
                    <div style="margin-top: 8px; text-align: right;">
                        <span
                            class="md3-chip <?php echo ($data['health']['disk']['percent_used'] ?? 0) > 90 ? 'error' : (($data['health']['disk']['percent_used'] ?? 0) > 75 ? 'warning' : 'success'); ?>">
        ...
        <div class="md3-card" style="margin-top: 24px;">
            ...
            <?php if (isset($data['health']['memory'])): ?>
                <div>
                    ...
                    <div class="md3-linear-progress">
                        <div class="md3-linear-progress-bar"
                            style="width: <?php echo $data['health']['memory']['percent_used'] ?? 0; ?>%;"></div>
                    </div>
                    <div style="margin-top: 8px; text-align: right;">
                        <span
                            class="md3-chip <?php echo ($data['health']['memory']['percent_used'] ?? 0) > 90 ? 'error' : (($data['health']['memory']['percent_used'] ?? 0) > 75 ? 'warning' : 'success'); ?>">
            ...
            <?php if (isset($data['health']['security'])): ?>
                <div>
                    <div style="display: flex; align-items: center; gap: 16px; margin-bottom: 16px;">
                        <div class="md3-stats-card-value" style="font-size: 2rem;">
                            <?php echo $data['health']['security']['score'] ?? 0; ?></div>
                        <div>
                            <div class="md3-stats-card-label">Security Score</div>
                            <span
                                class="md3-chip <?php echo ($data['health']['security']['status'] ?? '') === 'good' ? 'success' : 'warning'; ?>">
                            ...
                            <?php foreach ($data['health']['security']['checks'] as $check => $status): ?>
                                <li style="padding: 8px 0; display: flex; align-items: center; gap: 8px;">
                                    <span class="material-symbols-outlined"
                                        style="color: <?php echo $status ? 'var(--md-sys-color-primary)' : 'var(--md-sys-color-error)'; ?>;">
