		...
		<div class="widget-title dashboard-panel ">
			...
			<form action="" method="post">
				<ul>
					...
					<li><label>Backup prefix</label><input type="text" name="prefix" id="prefix"
							placeholder="Set backup prefix"
							value="<?php echo $data['default_settings']['prefix']; ?>" /></li>
					<li><label>Set autobackup interval</label><input type="text" name="interval" id="interval"
							placeholder="Min interval 1h" value="<?php echo $data['default_settings']['interval']; ?>">
					</li>
					<li><input type="checkbox" name="reset_interval" id="reset_interval" value="1" /> Reset Interval
					</li>
				</ul>
				<input type='submit' class='btn btn-blue' name="submit_autobackup" value="Save Autobackup">
				<?php echo CSRFProtection::get_token_field('autobackup'); ?>
			</form>
		</div>
	</div>
</div>