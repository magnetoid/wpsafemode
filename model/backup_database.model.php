...
class BackupDatabaseModel extends DashboardModel
{
	...
	public function backup_tables_csv($allowed_tables = '', $archive = false)
	{
		...
		foreach ($tables as $table) {
			...
			// SECURITY FIX: Use PHP fputcsv instead of SELECT INTO OUTFILE to avoid needing FILE privilege
			try {
				$fp = fopen($backup_file_csv, 'w');
				if (!$fp) {
					error_log('WP Safe Mode: Could not open file for writing: ' . $backup_file_csv);
					continue;
				}

				// Get data
				$q = $this->query("SELECT * FROM `" . DB_NAME . "`.`" . $validated_table . "`");
