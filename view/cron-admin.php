                ...
                <tr>
                    ...
                    <td>
                        <div style="display: flex; gap: 4px;">
                            <button class="md3-button md3-button-text" onclick="runCronJob('<?php echo htmlspecialchars($job['hook']); ?>')" title="Run Now">
                                <span class="material-symbols-outlined" style="margin-right: 4px; font-size: 18px;">play_arrow</span>
                                Run
                            </button>
                            <button class="md3-icon-button" onclick="deleteCronJob('<?php echo htmlspecialchars($job['hook']); ?>', <?php echo $job['timestamp']; ?>)" title="Delete" style="color: var(--md-sys-color-error);">
...
function runCronJob(hook) {
    ...
    WPSafeMode.Utils.showMessage('Running cron job: ' + hook, 'info');
...
function deleteCronJob(hook, timestamp) {
    ...
    WPSafeMode.Utils.showMessage('Cron job deletion coming soon', 'info');
    // TODO: Implement via API
}
</script>
