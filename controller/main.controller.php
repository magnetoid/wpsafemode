...
class MainController
{
	...
	function render(string $template = '', $data = '', $includes = ''): void
	{
		...
		// Check for admin template first
		$admin_template = $template . '-admin';
		$admin_path = $this->resolveTemplatePath($admin_template);

		// Cache file existence checks
		$cache_key_admin = $admin_path;
		if (!isset(self::$template_cache[$cache_key_admin])) {
			self::$template_cache[$cache_key_admin] = $admin_path && file_exists($admin_path);
		}

		if (self::$template_cache[$cache_key_admin]) {
			include $admin_path;
			return;
		}
